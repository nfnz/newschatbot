        ...
        for node, article_data in value.items():
            for q in article_data:
                ...
                if not exist:
                    ...
                    new_question = Questions(news_id=new_article.id, question_text=q['QUIZ']['QUIZ_TITLE'],
                                             question_type=1, order=1)
                    ...
                    for key, answer in q['QUIZ']['QUIZ_OPTIONS'].items():
                        for option in answer:
                            new_answer = Answers(question_id=new_question.id,
                                                 answer_text=option['OPTION_LABEL'],
                                                 correct_answer_text= "TBD",
                                                 correct_answers=True if option['CORRECT'] is not None else False,
                                                 order=1)
